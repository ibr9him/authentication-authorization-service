...
//    @WithMockUser(username = "admin", password = "password", authorities = ["CREATE_CLIENT"])
...
//        def content = new JsonSlurper().parseText(response.contentAsString)
        ...
        then:
        1 * mockClientService.save(_ as com.estore.authenticationauthorizationservice.client.dto.ClientCreationDto) >> ClientDto.builder().id(UUID.randomUUID()).build()
...
//
//        then:
//        1 * mockClientService.get("1") >> ClientBaseDto.builder().id("1").build()
//
//        expect:
//        response.status == OK.value()
//        content.id == "1"
...
//
//        then:
//        1 * mockClientService.getAll(_ as Pageable, "") >> { new PageImpl<>(List.of(ClientBaseDto.builder().build())) }
//
//        expect:
//        response.status == OK.value()
//    }
